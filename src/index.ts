...
const bot = new Bot(/*process.env.TG_TOKEN*/'7035011978:AAF8vnDTL-TN9u0fxTU2h7vK1xsnRPhg_ys', {
    polling: true
});
...
AppDataSource.initialize().then(async () => {
    ...
    bot.on('callback_query', async (q) => {
        ...
        if (q.data === 'about') {
        ...
        } else if (q.data === 'test' || q.data === 'program-y' || q.data === 'program-n') {
            if (user.complete) {
                await wait(0.5);
            ...
            const img: Buffer = fs.readFileSync(path.join(__dirname, 'img', 'massage.png'));
            await wait(0.5);
            ...
            await wait(0.5);
            await bot.sendMessage(q.from.id, '📍Наш адрес: [Мебельная д47 к1](https://yandex.ru/navi/-/CCUNjBd2TA)', {
    ...
    bot.onText(/./, async (msg) => {
        ...
        if (user && user.waitingFor === 'phone') {
            ...
            await userRepo.save(user);
            await bot.sendMessage(msg.from.id, 'Должно быть сообщение админу, телефон: ' + msg.text);
            ...
            await userRepo.save(user);
            await wait(0.5);
            await bot.sendMessage(msg.from.id, 'Записал. В ближайшее время с вами свяжется наш менеджер и проконсультирует по визиту 🩷');
            ...
            if (user.tellMoreQuery === 'Травмы') {
                await bot.sendMessage(msg.from.id, 'Есть ли у вас противопоказания к массажу?', {
                    reply_markup: {
                        inline_keyboard: [
                            [
                                {
                ...
                await bot.sendMessage(msg.from.id, 'Есть ли у вас хронические заболевания?', {
                    reply_markup: {
                        inline_keyboard: [
                            [
                                {
                ...
                await bot.sendMessage(msg.from.id, 'Какой цели ты бы хотел достичь через массаж?', {
                    reply_markup: {
                        inline_keyboard: [
                            [
                                {
                ...
                await bot.sendMessage(msg.from.id, 'На основе ваших ответов я бы предложил классический массаж');
                await bot.sendMessage(msg.from.id, 'Ну что, готовы прийти к нам на визит?', {
                    reply_markup: {
                        inline_keyboard: [
                            [
                                {
